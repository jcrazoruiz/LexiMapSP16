...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extrae texto nativo de los documentos seleccionados de un dominio médico. "
            "No utiliza OCR. Persiste exactamente el texto UTF-8 que posteriormente "
            "consumirá LexiMapSp-16."
        )
    )
    ...
    parser.add_argument(
        "--selection-csv",
        required=True,
        help="CSV de selección final del dominio."
    )

    parser.add_argument(
        "--specialty-path",
        required=True,
        help="Ruta raíz de la carpeta del dominio."
    )
    ...
    parser.add_argument(
        "--domain",
        required=True,
        help="Nombre lógico del dominio para mostrar en consola."
    )
...
def print_summary(
    ...
    print()
    print("=" * 80)
    print(f"EXTRACCION {domain.upper()} - RESULTADO FINAL")
    print("=" * 80)
...
def main() -> int:
    ...
    if not specialty_path.exists():
        print(
            f"ERROR: no existe la carpeta del dominio:\n"
            f"{specialty_path}"
        )
        return 1
    ...
    print()
    print("=" * 80)
    print(f"EXTRACCION NATIVA - {domain.upper()}")
    print("=" * 80)
